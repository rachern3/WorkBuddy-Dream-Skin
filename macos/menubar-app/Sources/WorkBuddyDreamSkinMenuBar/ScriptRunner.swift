...
enum ScriptRunner {
  static func run(script: URL, arguments: [String] = [], completion: @escaping (ScriptResult) -> Void) {
    DispatchQueue.global(qos: .userInitiated).async {
      ...
      do {
        ...
        result = ScriptResult(exitCode: process.terminationStatus, output: String(decoding: data, as: UTF8.self))
      } catch {
